...
bool queue_empty(queue_t* q) {
  return (q->indexIn == q->indexOut);       //returns true if indexIn and indexOut have the same value
}

queue_index_t incrementIndex(queue_t* q, queue_index_t index) {
   return (index + 1) % q->size;
}


// Returns true if the queue is full.
bool queue_full(queue_t* q) {return (incrementIndex(q,q->indexIn) == q->indexOut);}

//if the queue is not empty, remove and return the odlest element in the queue
...
queue_data_t queue_pop(queue_t* q) {
  if(queue_empty(q)) {                          //check if queue is empty
    q->underflowFlag = true;                    //if queue is empty, set underflowFlag
    printf("ERROR: failed to pop() because queue %s is empty\r\n", q->name);  //print out error message
    return QUEUE_RETURN_ERROR_VALUE;            //return error value
  }

  queue_data_t popVal = q->data[q->indexOut];   //if queue is non-empty, we will return the value at index indexOut
  q->indexOut = incrementIndex(q, q->indexOut); //increment the index of indexOut using modular arithmetic
  q->elementCount--;                            //decrement element count
  q->elementCount--;                            // track total number of elements
  return popVal;                                //return the correct value
}
...
void queue_push(queue_t* q, queue_data_t value){
  if (queue_full(q)){                               // check q is full
    q->overflowFlag = 1;                               // set overflowFlag
    printf("ERROR queue %s is full \r\n", q->name); // print error that 'queue name' is full
  }else{
    q->data[q->indexIn] = value;                      // update queue with new value
    q->indexIn = incrementIndex(q,q->indexIn);        // track index
    q->elementCount++;                                // track total number of elements
    q->underflowFlag = 0;                                // set underflowFlag
  }
}
...
void queue_overwritePush(queue_t* q, queue_data_t value){
    if (queue_full){queue_pop(q);}
  queue_push(q,value);
}

//provides random-access read capability to the queue
queue_data_t queue_readElementAt(queue_t* q, queue_index_t index) {
   //we will detect if an out of bounds error has occured
   if(index > q->elementCount) {
      //if so, we will notify the user
      printf("ERROR: Cannot access element %ld because queue %s only has %ld elements\n", index, q->name, q->elementCount);
      return QUEUE_RETURN_ERROR_VALUE;             //return queue error value
   }
   return q->data[(q->indexOut + index) % q->size];   //return value at specified index in queue
}

//returns if an underflow error has occured
bool queue_underflow(queue_t* q) {
   return q->underflowFlag;                        //returns true if an underflow error has occured
}

//returns true if an overflow error has occured
bool queue_overflow(queue_t* q) {
   return q->overflowFlag;                         //returns true if an overflow error has occured
}

// Returns a count of the elements currently contained in the queue.
queue_size_t queue_elementCount(queue_t* q){
  if (queue_full(q)){return queue_size(q);}           // check if queue is full
  else if (queue_empty){return 0;}                    // check if queue is empty
  return (q->elementCount);                           // if q in not empty or full
}
...
void queue_print(queue_t* q){
  for (uint32_t i = 0; i < q->elementCount; i++){
